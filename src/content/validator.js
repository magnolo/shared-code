...
import { isObject, cloneObject, roundTo } from "../utilities";
...
const styleReducer = (accumulator, obj) => {
  if (isObject(obj) && "fontFamily" in obj && "color" in obj)
    accumulator.push(obj);
  if (Array.isArray(obj) || isObject(obj))
    accumulator = accumulator.concat(getStyleObjects(obj));
  return accumulator;
};

const getStyleObjects = obj =>
  (Array.isArray(obj) ? obj : Object.values(obj)).reduce(styleReducer, []);

const formatReducer = (accumulator, obj) => {
  if (isObject(obj) && "formatType" in obj && "locale" in obj)
    accumulator.push(obj);
  if (Array.isArray(obj) || isObject(obj))
    accumulator = accumulator.concat(getFormatObjects(obj));
  return accumulator;
};

const getFormatObjects = obj =>
  (Array.isArray(obj) ? obj : Object.values(obj)).reduce(formatReducer, []);
...
export const getTooltipContent = content => {
    ...
    advContent.ops.push({
      ...
      insert: {
        inlineValue: {
          ...
          title: oldTitle,
          format: cloneObject(defaultFormat)
        }
      }
    });
  ...
  if (
    typeSpecific.tooltip.value.field &&
    typeSpecific.tooltip.value.field.includes("meta:")
  ) {
  ...
  const oldValueFormat = cloneObject(
    typeSpecific.tooltip.value.format || defaultFormat
  );
  ...
  if (
    oldValueFormat.prefix &&
    oldValueFormat.prefix.length > 0 &&
    oldValueFormat.prefix[oldValueFormat.prefix.length - 1] !== " "
  ) {
    oldValueFormat.prefix = oldValueFormat.prefix + " ";
  }

  if (
    oldValueFormat.suffix &&
    oldValueFormat.suffix.length > 0 &&
    oldValueFormat.suffix[0] !== " "
  ) {
  ...
  for (const field of typeSpecific.tooltip.fields) {
    if (field.field && field.field.length > 5) {
      ...
      if (oldAdditionalField && typeSpecific.fields[oldAdditionalField]) {
        const oldAdditionalTitle =
          typeSpecific.fields[oldAdditionalField].title;
        ...
        //patch prefix and suffix parts
        if (
          oldAdditionalFormat.prefix &&
          oldAdditionalFormat.prefix.length > 0 &&
          oldAdditionalFormat.prefix[oldAdditionalFormat.prefix.length - 1] !==
            " "
        ) {
          oldAdditionalFormat.prefix = oldAdditionalFormat.prefix + " ";
        }
        if (
          oldAdditionalFormat.suffix &&
          oldAdditionalFormat.suffix.length > 0 &&
          oldAdditionalFormat.suffix[0] !== " "
        ) {
  ...
  try {
    ...
    if (typeSpecific && typeSpecific.version && typeSpecific.version < 3) {
          ...
          if (field.dataType === "string") {
            ...
            if (field.isoIsoField) {
              ...
              if (typeSpecific.fields[field.isoIsoField].isoMappingId) {
                field.isoMappingId =
                  typeSpecific.fields[field.isoIsoField].isoMappingId;
              }
            }
          }
      ...
      if (visualTypes.includes(type) && !typeSpecific.dimensions) {
        ...
        for (const dimension of dimensionsList) {
          ...
          for (const fieldName of Object.keys(typeSpecific.fields)) {
            ...
            if (
              dimension.requirements.dataTypes &&
              !dimension.requirements.dataTypes.includes(field.dataType)
            ) {
              continue;
            }
            if (
      ...
      if (visualTypes.includes(type)) {
        ...
        typeSpecific.chart.animation.delay =
          typeSpecific.chart.animation.delay || 10;
        typeSpecific.chart.scrollable =
          typeSpecific.chart.scrollable === undefined
            ? true
            : typeSpecific.chart.scrollable;
        typeSpecific.chart.values.position =
          typeSpecific.chart.values.position || "auto";
        ...
        typeSpecific.chart.axis.labelAxis.format = cloneObject(
          typeSpecific.fields[labelField].format
        );
        typeSpecific.chart.axis.valueAxis.format = cloneObject(
          typeSpecific.fields[valueField].format
        );
      }
      ...
      typeSpecific.style.title =
        typeSpecific.style.title || getFormatObject(15, 500);
      typeSpecific.style.subTitle =
        typeSpecific.style.subTitle || getFormatObject(13, 300);
      typeSpecific.style.description =
        typeSpecific.style.description || getFormatObject(12, 300);
    ...
    if (typeSpecific && typeSpecific.version && typeSpecific.version < 4) {
      ...
      if (content.typeSpecific.updateOptions) {
        for (const key in content.typeSpecific.updateOptions) {
          ...
          content.typeSpecific.updateOptions[key] = JSON.parse(
            content.typeSpecific.updateOptions[key]
          );
        }
      } else {
        ...
        typeSpecific.map = {
          ...
          legend: {
            ...
            values: {
              format: JSON.parse(
                JSON.stringify(
                  typeSpecific.fields[getValueFieldName(content)].format
                )
              )
            },
            title: {
              format: JSON.parse(
                JSON.stringify(
                  typeSpecific.fields[getLabelFieldName(content)].format
                )
              ),
              value: null,
              label: null
      ...
      if (visualTypes.includes(type) && typeSpecific.map) {
        if (typeSpecific.map.legend.hide === undefined)
          typeSpecific.map.legend.hide = false;
        if (typeSpecific.map.legend.closed === undefined)
          typeSpecific.map.legend.closed = false;
        ...
        if (!typeSpecific.map.legend.values.format) {
          typeSpecific.map.legend.values.format = JSON.parse(
            JSON.stringify(
              typeSpecific.fields[getValueFieldName(content)].format
            )
          );
        ...
        if (!typeSpecific.map.legend.title.format) {
          typeSpecific.map.legend.title.format = JSON.parse(
            JSON.stringify(
              typeSpecific.fields[getLabelFieldName(content)].format
            )
          );
         ...
         *
         */
        if (!typeSpecific.map.options.hasOwnProperty("zoomButtons"))
          typeSpecific.map.options.zoomButtons = true;
        if (!typeSpecific.map.options.hasOwnProperty("fullscreen"))
          typeSpecific.map.options.fullscreen = false;
        if (!typeSpecific.map.options.hasOwnProperty("compass"))
          typeSpecific.map.options.compass = false;
        if (!typeSpecific.map.options.hasOwnProperty("scale"))
          typeSpecific.map.options.scale = false;
        if (!typeSpecific.map.options.hasOwnProperty("enableSearch"))
          typeSpecific.map.options.enableSearch = false;
        if (!typeSpecific.map.options.hasOwnProperty("extrude"))
          typeSpecific.map.options.extrude = false;
        if (!typeSpecific.map.options.opacity)
          typeSpecific.map.options.opacity = 1;
        if (!typeSpecific.map.options.outlinesColor)
          typeSpecific.map.options.outlinesColor = "rgba(0,0,0,0.2)";
    ...
    if (typeSpecific.version < 5) {
      ...
      if (content.type === "report") {
        content.typeSpecific.report.chapters = content.typeSpecific.report
          .chapters || [{ title: "Chapter 1", children: [] }];
        content.typeSpecific.report.chapterColumns =
          content.typeSpecific.report.chapterColumns || 1;

        // 15.01.2019
        typeSpecific.report.paginationType =
          typeSpecific.report.paginationType || "default";
        typeSpecific.report.paginationColor =
          typeSpecific.report.paginationColor || "#ffffff";
        typeSpecific.report.iconsColor =
          typeSpecific.report.iconsColor || "#ffffff";
        typeSpecific.report.progressBarColor =
          typeSpecific.report.progressBarColor || "#ffffff";
        typeSpecific.report.switchButton = typeSpecific.report.switchButton || {
          color: "#ffffff",
          backgroundColor: "rgba(0, 0, 0, 0.5);"
        };
        if (!typeSpecific.report.hasOwnProperty("showFullscreenButton"))
          typeSpecific.report.showFullscreenButton = true;
      }
      ...
      typeSpecific.style.subTitle.shadow = typeSpecific.style.subTitle
        .shadow || {
        x: 0,
        y: 0,
        blur: 0,
      ...
      typeSpecific.style.description.shadow = typeSpecific.style.description
        .shadow || {
        x: 0,
        y: 0,
        blur: 0,
      ...
      if (
        typeSpecific.updateOptions &&
        typeSpecific.updateOptions.hasOwnProperty("updatedNotification")
      ) {
        typeSpecific.updateOptions.updateNotification =
          typeSpecific.updateOptions.updatedNotification;
      ...
      if (typeSpecific.map && typeSpecific.map.legend) {
        typeSpecific.map.legend.type =
          typeSpecific.map.legend.type ||
          (typeSpecific.style.colors.type === "percentage"
            ? "gradient"
            : "static") ||
          "gradient";
      }
    ...
    if (typeSpecific.version < 6) {
      ...
      if (content.type === "report") {
        ...
        typeSpecific.style.description = cloneObject(
          extendedDescriptionFormatObject
        );
      } else {
        typeSpecific.style.title = convertFormatToExtendedFormat(
          typeSpecific.style.title,
          extendedTitleFormatObject
        );
        typeSpecific.style.subTitle = convertFormatToExtendedFormat(
          typeSpecific.style.subTitle,
          extendedSubTitleFormatObject
        );
        typeSpecific.style.description = convertFormatToExtendedFormat(
          typeSpecific.style.description,
          extendedDescriptionFormatObject
        );
      }
      ...
      if (typeSpecific.tooltip) {
        typeSpecific.tooltip.titleStyle =
          ...
          getExtendedFormatObject(
            ...
            undefined,
            undefined,
            getFormatMarginPaddingObject(0, undefined, 0, undefined, 5, "px")
          );
        typeSpecific.tooltip.valueStyle =
          typeSpecific.tooltip.valueStyle || getExtendedFormatObject(20, 300);
        typeSpecific.tooltip.contentStyle =
          typeSpecific.tooltip.contentStyle || getExtendedFormatObject();
      }

      // chart legend
      if (typeSpecific.chart && typeSpecific.chart.legend) {
        const margin = getFormatMarginPaddingObject(
          10,
          "px",
          10,
          "px",
      ...
      if (typeSpecific.chart && typeSpecific.chart.values) {
        typeSpecific.chart.values.style =
          typeSpecific.chart.values.style || getExtendedFormatObject(11);
      }

      // chart axis
      if (typeSpecific.chart && typeSpecific.chart.axis) {
        typeSpecific.chart.axis.labelAxis.titleStyle =
          typeSpecific.chart.axis.labelAxis.titleStyle ||
          getExtendedFormatObject(11, 500);
        typeSpecific.chart.axis.labelAxis.valueStyle =
          typeSpecific.chart.axis.labelAxis.valueStyle ||
          getExtendedFormatObject(11);

        typeSpecific.chart.axis.valueAxis.titleStyle =
          typeSpecific.chart.axis.valueAxis.titleStyle ||
          getExtendedFormatObject(11, 500);
        typeSpecific.chart.axis.valueAxis.valueStyle =
          typeSpecific.chart.axis.valueAxis.valueStyle ||
          getExtendedFormatObject(11);
      }
      ...
      // map legend
      if (typeSpecific.map && typeSpecific.map.legend) {
        typeSpecific.map.legend.titleStyle =
          typeSpecific.map.legend.titleStyle || getExtendedFormatObject();
      ...
      if (content.type === "report") {
        typeSpecific.report.visualDescriptionTitle =
          typeSpecific.report.visualDescriptionTitle ||
          getExtendedFormatObject(16, 500, undefined, "#ffffff");
        typeSpecific.report.visualDescription =
          ...
          getExtendedFormatObject(
            ...
            undefined,
            undefined,
            getFormatMarginPaddingObject(10, "px")
          );
      }
    ...
    if (typeSpecific.version < 8) {
      ...
      if (content.type === "report") {
        ...
        typeSpecific.report.paginationStyle =
          typeSpecific.report.paginationStyle ||
          getExtendedFormatObject(18, 300, undefined, "#ffffff");
      }
    ...
    if (typeSpecific.version < 9) {
      ...
      if (visualTypes.includes(type)) {
        ...
        typeSpecific.map.options.bearing =
          typeSpecific.map.options.bearing || 0;
        typeSpecific.map.options.displayType =
          typeSpecific.map.options.displayType || "choropleth";
        typeSpecific.map.options.outlinesWidth =
          typeSpecific.map.options.outlinesWidth || 0;
        ...
        typeSpecific.map.options.extrude =
          typeof typeSpecific.map.options.extrude === "object"
            ? typeSpecific.map.options.extrude
            : { height: 10000 };
        typeSpecific.map.options.extrude.scaling =
          typeSpecific.map.options.extrude.scaling || "linear";
        typeSpecific.map.options.showAllSourceFeatures =
          typeSpecific.map.options.showAllSourceFeatures || false;
        typeSpecific.map.options.emptyFeaturesColor =
          typeSpecific.map.options.emptyFeaturesColor || "#ccc";
      }
    ...
    if (typeSpecific.version < 11) {
      ...
      if (typeSpecific.style.colors) {
        ...
        if (typeSpecific.map.legend) {
          // NEW LEGEND STUFF
          const hasOldOverrides =
            colorsCount > 0
              ? oldColors.match.filter(match => match.title).length > 0
              : false;
          const oldMatchTitles = hasOldOverrides
            ? oldColors.match.map(match => match.title)
            : [];
          const overrideValues = oldMatchTitles.length ? oldMatchTitles : [];
          if (typeSpecific.map.legend.values) {
            typeSpecific.map.legend.values.overrides = typeSpecific.map.legend
              .values.overrides || {
      ...
      if (content.type === "report") {
        ...
        typeSpecific.report.switchFont =
          typeSpecific.report.switchFont ||
          getExtendedFormatObject(12, 500, "center", colorDefault);
    ...
    if (typeSpecific.version < 13) {
      if (visualTypes.includes(content.type)) {
        typeSpecific.chart.legend.scrollable = typeSpecific.chart.legend.hasOwnProperty(
          "scrollable"
        )
          ? typeSpecific.chart.legend.scrollable
          : true;
        typeSpecific.chart.legend.scrollHeight = typeSpecific.chart.legend.hasOwnProperty(
          "scrollHeight"
        )
          ? typeSpecific.chart.legend.scrollHeight
          : 30;
      }
    ...
    if (typeSpecific.version < 14) {
      ...
      if (
        typeSpecific.inVisualFilter &&
        typeSpecific.inVisualFilter.length > 0
      ) {
      ...
      typeSpecific.style.inVisualFilter =
        typeSpecific.style.inVisualFilter || defaultInVisStyle;

      if (content.type === "report") {
        content.typeSpecific.report.activeChapterBackground =
          content.typeSpecific.report.activeChapterBackground ||
          "rgba(0,0,0,0.5)";
      }
    ...
    if (typeSpecific.version < 15) {
      ...
      if (content.type === "report") {
        ...
        typeSpecific.report.chaptersWidth = typeSpecific.report
          .chaptersWidth || {
          mobile: {
            unit: "%",
            value: 90
        ...
        typeSpecific.report.activeChapterWidth = typeSpecific.report
          .activeChapterWidth || {
          viz: {
            unit: "%",
            value: 70
    ...
    if (typeSpecific.version < 16) {
      if (visualTypes.includes(type) && typeSpecific.map) {
        typeSpecific.map.options.animationSpeed =
          typeSpecific.map.options.animationSpeed || 1.2;
        typeSpecific.map.options.animationCurve =
          typeSpecific.map.options.animationCurve || 1.42;
      }
    ...
    if (typeSpecific.version < 17) {
      if (visualTypes.includes(type) && typeSpecific.chart) {
        const padding = getFormatMarginPaddingObject(
          10,
          "px",
          10,
          "px",
    ...
    if (typeSpecific.version < 18) {
      const styleObjects = getStyleObjects(content);
      for (const obj of styleObjects) {
        obj.fontFamily = obj.fontFamily.replace(
          "Merriweather Italic",
          "Merriweather"
        );
        obj.fontFamily = obj.fontFamily.replace(
      ...
      if (content.type === "map") {
        typeSpecific.map.searchField = typeSpecific.map.searchField
          ? typeSpecific.map.searchField
          : "default";
      }
      ...
      if (visualTypes.includes(type)) {
        ...
        const closeButtonPadding = getFormatMarginPaddingObject(
          5,
          "px",
          5,
          "px",
        ...
        const closeButtonMargin = getFormatMarginPaddingObject(
          5,
          "px",
          5,
          "px",
      ...
      for (const breakPoint of breakPoints.values) {
        breakPoint.aspectRatio =
          breakPoint.aspectRatio || roundTo(aspectScale(breakPoint.domain), 3);
      }
    ...
    if (typeSpecific.version < 19) {
      if (visualTypes.includes(content.type)) {
        typeSpecific.chart.values.darkColor =
          typeSpecific.chart.values.darkColor || "#000000";
        typeSpecific.chart.values.lightColor =
          typeSpecific.chart.values.lightColor || "#ffffff";

        typeSpecific.chart.grid.width =
          "width" in typeSpecific.chart.grid
            ? typeSpecific.chart.grid.width
            : 1;
        typeSpecific.chart.scaleZero =
          "scaleZero" in typeSpecific.chart
            ? typeSpecific.chart.scaleZero
            : true;
    ...
    if (typeSpecific.version < 20) {
      if (visualTypes.includes(content.type)) {
        typeSpecific.style.descriptionBackground = typeSpecific.style
      ...
      if (visualTypes.includes(type)) {
        typeSpecific.layout =
          typeSpecific.layout || createVisualLayout(content); // typeSpecific.layout ||

        // apply custom layout to tagesschau maps
        if (
          content.userInfo.email === "tagesschau@23degrees.io" &&
          type === "choropleth"
        ) {
          const activeLayout =
            typeSpecific.layout.layouts[typeSpecific.layout.active];
          ...
          if (activeLayout.autoGenerated) {
            ...
            activeLayout.children[2].children[0].style.alignItems =
              "flex-start";

            const logoChild = activeLayout.children[2].children[0].children.splice(
              0,
              1
            )[0];
            logoChild.options.margin.left = 20;
            logoChild.options.margin.bottom = 10;
            activeLayout.children[1].children[1].children[2].children.unshift(
              logoChild
            );
            activeLayout.children[1].children[1].children[2].style.justifyContent =
              "space-between";
    ...
    if (typeSpecific.version < 21) {
      if (visualTypes.includes(type) || type === "atlas" || type === "report") {
        ...
        // HOTFIX 28.06.2019
        if (typeSpecific.export.external === true)
          typeSpecific.embed.mode = "external";
        if (typeSpecific.export.stretch === false)
          typeSpecific.embed.mode = "fixed";
        ...
        if (content.injected) {
          ...
          if (breakies.length === 5) {
            ...
            if (
              ...
              (breakies[1].domain === 600 && breakies[1].range === 1) &&
              (breakies[2].domain === 800 && breakies[2].range === 1.2) &&
              (breakies[3].domain === 1280 && breakies[3].range === 1.5) &&
              (breakies[4].domain === 1920 && breakies[4].range === 2)
            ) {
              ...
              typeSpecific.style.breakPoints.version =
                typeSpecific.style.breakPoints.version || 1;
            }
          }
        }
      }
    ...
    if (typeSpecific.version < 22) {
      if (type === "dataset") {
        typeSpecific.autocreateGroups = typeSpecific.hasOwnProperty(
          "autocreateGroups"
        )
          ? typeSpecific.autocreateGroups
          : false;
    ...
    if (typeSpecific.version < 24) {
      if (visualTypes.includes(type)) {
        const attributionMargin = getFormatMarginPaddingObject(
          3,
          "px",
          3,
          "px",
        ...
        typeSpecific.map.legend.showOnlyOccurrences = typeSpecific.map.legend.hasOwnProperty(
          "showOnlyOccurrences"
        )
    ...
    if (typeSpecific.version < 26) {
      ...
      if (visualTypes.includes(type) && typeSpecific.map) {
        ...
        typeSpecific.map.options.highlight = typeSpecific.map.options
          .highlight || {
          enabled: true,
          type: "line",
          width: 1,
    ...
    if (typeSpecific.version < 27) {
      if (visualTypes.includes(type) && typeSpecific.map) {
        typeSpecific.map.options.renderWorldCopies = typeSpecific.map.options.hasOwnProperty(
          "renderWorldCopies"
        )
    ...
    if (typeSpecific.version < 28) {
      ...
      if (visualTypes.includes(type) && typeSpecific.map.options) {
        typeSpecific.map.options.searchPlaceholder = typeSpecific.map.options
          .searchPlaceholder
          ? typeSpecific.map.options.searchPlaceholder
          : "";
      }

      if (visualTypes.includes(type) && typeSpecific.tooltip) {
        typeSpecific.tooltip.theme = typeSpecific.tooltip.theme
          ? typeSpecific.tooltip.theme
          : "default";
      }
      ...
      if (visualTypes.includes(type)) {
        if (
          ...
          const mobileHeightValue =
            typeSpecific.advancedTooltip.style.classic.maxHeight.mobile.value;
          const desktopHeightValue =
            typeSpecific.advancedTooltip.style.classic.maxHeight.desktop.value;
    ...
    if (typeSpecific.version < 29) {
      if (visualTypes.includes(type)) {
        ...
        if (!!typeSpecific.chart.background)
          delete typeSpecific.chart.background;
        ...
        if (typeSpecific.map.options) {
          ...
          typeSpecific.map.options.withStartAnimation = typeSpecific.map.options.hasOwnProperty(
            "withStartAnimation"
          )
            ? typeSpecific.map.options.withStartAnimation
            : !!options.flyTo || !options.fitBounds;
          typeSpecific.map.options.startViewport =
            typeSpecific.map.options.startViewport ||
            options.fitBounds ||
            options.flyTo;
          ...
          if (
            startViewport &&
            Array.isArray(startViewport) &&
            startViewport.length > 2
          ) {
            typeSpecific.map.options.startViewport = [
          ...
          typeSpecific.map.options.maxViewport = typeSpecific.map.options
            .maxViewport ||
            options.maxBounds || [[-180, -85], [180, 85]];
          typeSpecific.map.options.maxViewportIsLimit = typeSpecific.map.options.hasOwnProperty(
            "maxViewportIsLimit"
          )
            ? typeSpecific.map.options.maxViewportIsLimit
            : !!options.maxBounds;

          if (!!typeSpecific.map.options.fitBounds)
            delete typeSpecific.map.options.fitBounds;
          if (!!typeSpecific.map.options.flyTo)
            delete typeSpecific.map.options.flyTo;
          if (!!typeSpecific.map.options.maxBounds)
            delete typeSpecific.map.options.maxBounds;
        }
      }
    ...
    if (typeSpecific.version < 30) {
      ...
      if (visualTypes.includes(type)) {
        ...
        if (
          typeSpecific.tooltip &&
          typeSpecific.tooltip.value &&
          typeSpecific.tooltip.value.field
        ) {
          const tooltipField = typeSpecific.tooltip.value.field;
          if (tooltipField && tooltipField.startsWith("data:")) {
            const tooltipFieldName = tooltipField.substr(
              tooltipField.indexOf(":") + 1
            );
            const parentField = typeSpecific.fields[tooltipFieldName];
            if (
              parentField &&
              parentField.format &&
              typeSpecific.tooltip.value.field.format
            ) {
              typeSpecific.tooltip.value.format.formatType =
                parentField.format.formatType;
            }
          }
        }
        ...
        if (
          typeSpecific.tooltip &&
          typeSpecific.tooltip.fields &&
          typeSpecific.tooltip.fields.length > 0
        ) {
          for (const field of typeSpecific.tooltip.fields) {
        ...
        if (valueFieldName) {
          ...
          if (parentField) {
            ...
            if (
              typeSpecific.map &&
              typeSpecific.map.legend &&
              typeSpecific.map.legend.title &&
              typeSpecific.map.legend.title.format
            ) {
              typeSpecific.map.legend.title.format.formatType =
                parentField.format.formatType;
            }
            //legend values
            if (
              ...
              typeSpecific.map.legend &&
              typeSpecific.map.legend.values &&
              typeSpecific.map.legend.values.format
            ) {
              typeSpecific.map.legend.values.format.formatType =
                parentField.format.formatType;
            }
            //chart values
            if (
              typeSpecific.chart &&
              typeSpecific.chart.values &&
              typeSpecific.chart.values.format
            ) {
              typeSpecific.chart.values.format.formatType =
                parentField.format.formatType;
            }
            //chart value axis
            if (
              typeSpecific.chart &&
              typeSpecific.chart.axis &&
              typeSpecific.chart.axis.valueAxis &&
              typeSpecific.chart.axis.valueAxis.format
            ) {
              typeSpecific.chart.axis.valueAxis.format.formatType =
                parentField.format.formatType;
            }
          }
        }
        ...
        if (labelFieldName) {
          ...
          if (parentField) {
            if (
              typeSpecific.chart &&
              typeSpecific.chart.axis &&
              typeSpecific.chart.axis.labelAxis &&
              typeSpecific.chart.axis.labelAxis.format
            ) {
              typeSpecific.chart.axis.labelAxis.format.formatType =
                parentField.format.formatType;
            }
          }
        }
